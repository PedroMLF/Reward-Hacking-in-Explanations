...
#SBATCH -o outputs/wino/llama3-8B/bon/log_bon_extra.txt
...
# Args
SAVE_PATH="outputs/wino/llama3-8B/bon/"
DATA_PATH="data/wino/"
        ...
        for SPLIT in orig cf; do
            for RW_MODEL in ${RW_MODEL_1} ${RW_MODEL_2}; do
                python ${PROJECT_HOME}/eval_rewards.py \
                    ${SAVE_PATH}/bias_${APPROACH}_${SPLIT}_test_data.joblib \
                    ${RW_MODEL} \
                    none \
                    --batch_size 32 \
                    --seed 0
...
if ${RUN_REWARD_AUGMENTED}; then
    ...
    for APPROACH in negative; do
        for RW_MODEL in ${RW_MODEL_1} ${RW_MODEL_2}; do
            python ${PROJECT_HOME}/eval_rewards.py \
                ${SAVE_PATH}/bias_${APPROACH}_orig_test_data.joblib \
                ${RW_MODEL} \
                heuristic \
                --batch_size 32 \
                --pcf_path ${SAVE_PATH}/bias_${APPROACH}_cf_test_data.joblib \
                --setting "wino" \
                --seed 0 \
                --do_all_augmented
            date '+[%H:%M:%S-%d/%m/%y]'; echo -e "\n\n"
