...
#SBATCH -o outputs/aqua/llama3-3B/bon/log_bon_reward.txt

# --------------------------------------------------------

# Args
SAVE_PATH="outputs/aqua/llama3-3B/bon/"
...
for SEED in 0 1 2; do
    ...
    for APPROACH in negative; do
        for RW_MODEL in ${RW_MODEL_1} ${RW_MODEL_2}; do
            ...
            python -u ${PROJECT_HOME}/get_rewards_data.py \
                ${SAVE_PATH_SEED}/cot_${APPROACH}_orig_test_data.joblib \
                ${RW_MODEL} \
                --batch_size ${BSIZE} \
                --setting aqua \
                --do_augmented \
    ...
    for RW in sk-llama sk-gemma; do
        python -u ${PROJECT_HOME}/eval_rewards.py \
            --pcf_path ${SAVE_PATH_SEED}/cot_negative_cf_test_data_${RW}_original.joblib \
            --rm_orig_path ${SAVE_PATH_SEED}/cot_negative_orig_test_data_${RW}_original.joblib \
            --rm_aug_path ${SAVE_PATH_SEED}/cot_negative_orig_test_data_${RW}_augmented.joblib \
            --seed ${SEED}
    done
